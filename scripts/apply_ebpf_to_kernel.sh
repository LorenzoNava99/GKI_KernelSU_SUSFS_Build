#!/bin/bash
# Script to apply full eBPF kernel configuration to GKI defconfig
# This script is used by the GitHub Actions workflow to enable eBPF features

set -e

CONFIG_FILE="${1:-./common/arch/arm64/configs/gki_defconfig}"
...
# Verify critical configs were added
echo ""
echo "Verifying critical eBPF configs..."
critical_configs=(
    "CONFIG_BPF_SYSCALL"
    "CONFIG_KPROBES"
    "CONFIG_UPROBES"
...
for config in "${critical_configs[@]}"; do
    if grep -q "^${config}=y" "$CONFIG_FILE"; then
        ...
        echo "✗ WARNING: ${config} not enabled!"
    fi
done
